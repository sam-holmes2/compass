...
**The Boss** (★) always appears first, with the same fields as any enemy, plus `whyBoss` and up to 3 vulnerabilities (specific, actionable moments the pattern can be interrupted). Bosses own sub-enemies that are faster-moving manifestations of the same root.
...
**In the JSON** (safe as long as you export): everything the AI writes, including quests, enemies, beliefs, skills, elements, journal, XP, and achievements. Export after every session.
