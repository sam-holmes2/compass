...
- **Only `data.json` is sensitive.** `character-sheet.html` and `instructions.md` are the same for every user -- share them freely. `data.json` holds everything you've journalled; treat it like a private diary.
- Everything stays on your machine by default. Nothing is sent anywhere unless you paste it into an AI.
...
- **Use [Firefox](https://www.firefox.com/) if possible.** In Chrome and Edge, all local HTML files share the same storage origin, so any other local HTML file you open could technically read your data. Firefox gives each local file its own isolated origin. If you prefer another browser and care about this, run character-sheet from a local server: install Python and run `python3 -m http.server --bind 127.0.0.1` in the same folder, then open `localhost:8000/character-sheet.html`. (The `--bind 127.0.0.1` flag is important -- without it, the server is reachable by others on your local network.)
...
- **Cloud storage** (Dropbox, Google Drive, iCloud) is fine for `character-sheet.html` and `instructions.md`. Think twice before syncing `data.json` to unencrypted cloud storage.
