...
- **Cloud AI (Anthropic, OpenAI, Google, Mistral):** Your journal content is sent to their servers and may be stored for weeks to years. Most providers let you opt out of training data use, but storage is separate from training.
...
**Frontier models are not available.** Claude, GPT-4, and Gemini cannot run in confidential infrastructure because their weights are proprietary. All current providers use open-source models: Llama, Gemma, Mistral, and similar. The quality gap for self-reflection work is real and worth weighing, particularly if you use AI as a genuine reflection partner rather than a data-entry assistant.
