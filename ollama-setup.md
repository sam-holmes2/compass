...
Everything you need to install, configure, and troubleshoot Ollama for use with character-sheet.
...
**Context window:** your character sheet data grows over time. A model's context window must be large enough to fit your data plus the conversation. Export your JSON from the app (click the download icon), then check the file size. A 50 KB file is roughly 12,000 tokens. Set the Context Window in Chat Settings to at least twice the size of your data. If your data is large, try a compact export or pick a model with a longer context window.
...
Open `character-sheet.html`, click the chat icon in the bottom bar, then the gear icon (Chat Settings). Your model should appear in the model list. Select it and start a session.
